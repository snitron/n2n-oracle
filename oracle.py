...
with open(os.environ['ORACLE_DATA'], 'r') as f:
    data = f.read()
    if data:
        os.environ['LEFT_START_BLOCK'] = max(int(json.loads(data)['left']), l_start)
with open(os.environ['ORACLE_DATA'], 'r') as f:
    data = f.read()
    if data:
        os.environ['RIGHT_START_BLOCK'] = max(int(json.loads(data)['left']), r_start)
...
os.environ['RIGHT_START_BLOCK'] = r_last
...
os.environ['LEFT_START_BLOCK'] = l_last
    ...
    while True:
        ...
        for log_l in event_filter.get_new_entries():
            ...
            hsh = send_update(amount, recipient, w3_right.eth.getTransactionCount(right_account.address),
                              txh, os.environ['RIGHT_GASPRICE'], os.environ['RIGHT_ADDRESS'],
                              w3_right, right_account)
        os.environ['LEFT_START_BLOCK'] = w3_left.eth.get_block('latest')['number']
...
def log_loop_right(event_filter, poll_interval):
    while True:
        ...
        os.environ['RIGHT_START_BLOCK'] = w3_right.eth.get_block('latest')['number']
